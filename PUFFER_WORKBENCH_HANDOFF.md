...
- Service Worker 安装阶段只原子缓存可离线启动页面的核心壳，任一核心文件失败时不接管旧版本；
- 新 Worker 激活后由页面在空闲时发送 `puffer-warm-cache`，先分批预热首屏图片，再分批预热本地字体；单个可选资源失败不会阻塞其他资源，下一次只重试缺失项；
- 未预热但实际被访问的 `assets/*` 仍会按需补入当前版本缓存；PWA 核心脚本使用 network-first，网络失败时才回退缓存。
...
- 前台发现对方新留言时显示可点击的站内消息卡；留言弹层已经打开时会原地更新对话，并保留未发送草稿和已选图片。未读数仅保存在当前设备，打开留言后立即清零。
