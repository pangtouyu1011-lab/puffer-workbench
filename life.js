(() => {
  ...
  let reactionSeenScope = '';
  let reactionSeen = null;
  let deferredReactionNotice = '';
  ...
  async function handleLifeSubmission(button) {
    return runLifeSubmission(button,async()=>{
      ...
      if(button.matches('[data-save-hydration]')){const kind=button.dataset.saveHydration==='drink'?'drink':'water',ok=window.PufferLife?.addHydration?.(kind,mask.querySelector('#lifeHydrationMl')?.value);if(ok){const me=state()?.settings?.me||'a',record=live(state()?.hydrationLog).filter(item=>item.author===me&&item.kind===kind&&item.date===dayKey()).sort((a,b)=>(b.createdAt||0)-(a.createdAt||0))[0];clearSheetDraft(button);closeSheet();requestHydrationReaction(record?.id||`${dayKey()}:${me}:${kind}`);}return !!ok;}
  ...
  const DAILY_ALMANAC_MOCK = Object.freeze({ lunarDate:'七月初八', suitable:['出行','会友','纳财'], avoid:['动土','嫁娶','开仓'] });
  function weatherAlmanac() { const source=DAILY_ALMANAC_MOCK; if(!source?.lunarDate)return null; const items=list=>Array.isArray(list)?list.map(item=>String(item||'').trim()).filter(Boolean).slice(0,3):[], suitable=items(source.suitable), avoid=items(source.avoid); if(!suitable.length&&!avoid.length)return null; return { lunarDate:String(source.lunarDate).trim(), suitable, avoid }; }
  function weatherAlmanacRow(kind, items) { if(!items.length)return ''; const text=items.join(' · '); return `<div class="life-weather-almanac-row"><span class="life-weather-almanac-label ${kind==='good'?'is-good':'is-bad'}">${kind==='good'?'宜':'忌'}</span><span class="life-weather-almanac-text" title="${esc(text)}">${esc(text)}</span></div>`; }
  ...
  function reactionScope() {
    const current=state(),room=current?.settings?.room,me=current?.settings?.me||'a';
    return `puffer-life-reactions:v1:${encodeURIComponent(room?.joined&&room.id?room.id:'local')}:${me}`;
  }
  function getReactionSeen() {
    const key=reactionScope();
    if(key!==reactionSeenScope){
      reactionSeenScope=key;reactionSeen=null;
    }
    if(!reactionSeen){
      try{reactionSeen=JSON.parse(localStorage.getItem(key)||'null');}catch(_){reactionSeen=null;}
      if(!reactionSeen||reactionSeen.date!==dayKey())reactionSeen={date:dayKey(),actions:{},hydrationMilestones:{},remoteActions:{},celebrationShown:false,remoteNotice:''};
      reactionSeen.actions=reactionSeen.actions||{};reactionSeen.hydrationMilestones=reactionSeen.hydrationMilestones||{};reactionSeen.remoteActions=reactionSeen.remoteActions||{};
    }
    return reactionSeen;
  }
  function saveReactionSeen() { try{localStorage.setItem(reactionScope(),JSON.stringify(getReactionSeen()));}catch(_){} }
  function takeReaction(bucket, id) {
    const seen=getReactionSeen(),group=seen[bucket]||(seen[bucket]={}),key=String(id||'');
    if(!key||group[key])return false;
    group[key]=Date.now();saveReactionSeen();return true;
  }
  function deferReactionNotice(copy) { const seen=getReactionSeen();seen.remoteNotice=String(copy||'');saveReactionSeen();deferredReactionNotice=seen.remoteNotice; }
  function showDeferredReactionNotice() {
    const seen=getReactionSeen(),copy=deferredReactionNotice||seen.remoteNotice;
    if(!copy||document.hidden)return false;
    deferredReactionNotice='';seen.remoteNotice='';saveReactionSeen();window.PufferLife?.notify?.(copy,'info');return true;
  }
  function playCompanionReaction(kind, options={}) {
    const config={
      ...
      milestone:{asset:'puffer-reaction-hydration-v1.png',tone:'happy',label:'喝水鼓励',text:options.text||'继续喝水，慢慢靠近今天的目标。'},
      celebrate:{asset:'puffer-state-celebrate.webp',tone:'celebrate',label:'庆祝胖头鱼',text:options.text||'今天一起完成啦'}
    }[kind];
    ...
    node._reactionTimer=setTimeout(()=>{
      ...
      feedback.remove();
      renderCompanionFloat(state());
    },Math.max(900,Math.min(1500,Number(options.duration)||1200)));
    return true;
  }
  function requestCompanionReaction(kind, actionId, options={}) {
    if(kind!=='celebrate'&&window.PufferLife?.isTodayInteractionComplete?.()&&getReactionSeen().celebrationShown)return false;
    if(!takeReaction('actions',actionId))return false;
    if(document.hidden){deferReactionNotice(options.deferredText||'刚刚有一件小事被胖头鱼记下了。');return false;}
    requestAnimationFrame(()=>playCompanionReaction(kind,options));
    return true;
  }
  function requestHydrationReaction(actionId) {
    const hydration=window.PufferLife?.getHydrationToday?.(),total=Number(hydration?.me?.water)||0;
    const milestone=[1500,1000,500].find(value=>total>=value&&!getReactionSeen().hydrationMilestones[value]);
    if(milestone){
      takeReaction('actions',actionId);
      getReactionSeen().hydrationMilestones[milestone]=Date.now();saveReactionSeen();
      if(document.hidden){deferReactionNotice(milestone===1500?'今天的喝水目标完成啦':'喝水进度又前进了一步。');return false;}
      return requestAnimationFrame(()=>playCompanionReaction(milestone===1500?'celebrate':'milestone',{duration:milestone===1500?1450:1100,text:milestone===1500?'喝水目标完成啦':'继续喝水，慢慢靠近今天的目标。'})),true;
    }
    return requestCompanionReaction('hydration',actionId);
  }
  function processRemoteCompanionReactions(changes) {
    const current=state(),me=current?.settings?.me||'a',ta=me==='a'?'b':'a',today=dayKey(),allowed=new Set(Array.isArray(changes)?changes:[]),candidates=[];
    const todayItem=item=>sameDay(item?.createdAt||item?.updatedAt);
    if(allowed.has('messages'))live(current?.messages).filter(item=>item.author===ta&&todayItem(item)).forEach(item=>candidates.push({id:`message:${item.id}`,kind:'message',copy:'TA 刚刚留了一句话。'}));
    if(allowed.has('gallery'))live(current?.gallery).filter(item=>item.author===ta&&todayItem(item)).forEach(item=>candidates.push({id:`photo:${item.id}`,kind:'photo',copy:'TA 刚刚收下一张照片。'}));
    if(allowed.has('hydration'))live(current?.hydrationLog).filter(item=>item.author===ta&&item.kind==='water'&&item.date===today).forEach(item=>candidates.push({id:`hydration:${item.id}`,kind:'hydration',copy:'TA 刚刚记下了一次喝水。'}));
    if(allowed.has('todo'))live(current?.todos).filter(item=>item.author===ta&&item.done&&item.date===today).forEach(item=>candidates.push({id:`todo:${item.id}:${item.updatedAt||item.createdAt}`,kind:'todo',copy:'TA 刚刚完成了一件待办。'}));
    if(allowed.has('mood')&&current?.dailyStatus?.[today]?.[ta])candidates.push({id:`mood:${today}:${ta}`,kind:'mood',mood:current.dailyStatus[today][ta].mood,copy:'TA 刚刚更新了今天的心情。'});
    const next=candidates.sort((a,b)=>String(b.id).localeCompare(String(a.id))).filter(item=>takeReaction('remoteActions',item.id))[0];
    if(!next)return false;
    if(document.hidden){deferReactionNotice(next.copy);return false;}
    requestAnimationFrame(()=>playCompanionReaction(next.kind,{mood:next.mood,text:next.copy}));return true;
  }
  function syncTodayInteractionCelebration() {
    if(!window.PufferLife?.isTodayInteractionComplete?.())return false;
    const seen=getReactionSeen();
    if(seen.celebrationShown)return false;
    seen.celebrationShown=true;saveReactionSeen();
    if(document.hidden){deferReactionNotice('今天一起完成啦');return false;}
    requestAnimationFrame(()=>playCompanionReaction('celebrate',{duration:1450,text:'今天一起完成啦'}));return true;
  }
  ...
  function renderToday(s) {
    ...
    const music = {};
    const p = s.settings?.partners || {}, me = s.settings?.me || 'a', ta = me === 'a' ? 'b' : 'a', weather = weatherInfo(s), almanac = weatherAlmanac(), todos = live(s.todos).filter(t => (!t.done && (!t.date || t.date === dayKey())) || (t.done && (t.date === dayKey() || (!t.date && sameDay(t.updatedAt))))).sort((a,b)=>Number(a.done)-Number(b.done)||(b.updatedAt||b.createdAt||0)-(a.updatedAt||a.createdAt||0)).slice(0, 3), mine = participation(s, me), theirs = participation(s, ta), photo = live(s.gallery).filter(x => x.dataUrl || x.url).sort((a,b) => (b.createdAt||0)-(a.createdAt||0))[0], latestMessage = live(s.messages).filter(m => m.author === ta && sameDay(m.createdAt)).sort((a,b) => (b.createdAt||0)-(a.createdAt||0))[0], status = s.dailyStatus?.[dayKey()]?.[ta], fortune = s.fortune?.date === dayKey() ? s.fortune.by?.[ta] : null;
    ...
    const dots = (items, owner) => items.map(item => `<button class="life-participation-item ${item.done?'is-done':''}" data-life-participation="${item.key}" data-life-person="${owner}" aria-label="${esc(item.label)}，${item.done?'已完成':'未完成'}" aria-pressed="${item.done?'true':'false'}" title="${esc(item.label)}"><span>${icon(item.key==='fortune'?'scroll':item.key==='mood'?'smiley':item.key==='message'?'chat-circle-text':'check-square')}</span><i>${item.done ? icon('check') : icon('plus')}</i></button>`).join('');
    ...
    const partnerToday = `<section class="life-section life-partner-today"><div class="life-section-head"><h2 class="life-section-title">${esc(p[ta] || 'TA')} 的今天</h2><span class="life-label">已同步的今日状态</span></div><div class="life-card life-partner-summary"><div class="life-partner-summary-grid"><button type="button" class="life-partner-mood life-partner-summary-compact" data-life-open="partner-mood" aria-label="查看对方今天的心情"><span>${icon('smiley')}</span><div><small>现在心情</small><b>${esc(status?.mood || '还没记录')}</b></div></button><article class="life-partner-summary-compact"><span>${icon('scroll')}</span><div><small>今日抽签</small><b>${esc(fortune ? `${fortune.level}签 · ${fortune.text}` : '还没抽签')}</b></div></article></div><button class="life-partner-insight" data-life-open="horoscope"><span>${icon('sparkle')}</span><div><small>星座相处提醒</small><b>${esc(partnerHoro?.data?.love || '今天的双人运势正在准备。')}</b></div>${icon('caret-right')}</button><button class="life-partner-note" data-life-open="messages"><span>${icon('chat-circle-text')}</span><div><small>想对你说</small><b>${esc(partnerNote || 'TA 还没有留下一句话。')}</b></div>${icon('caret-right')}</button></div></section>`;
    ...
    const interactionSlot=slot('today'),interaction=interactionSlot?.querySelector('.life-today-together'),partnerSection=interactionSlot?.querySelector('.life-partner-today'),challenge=todayChallenge(s),mineReady=mine.every(item=>item.done)&&!!challenge.mine,interactionComplete=!!window.PufferLife?.isTodayInteractionComplete?.();if(interaction&&partnerSection){partnerSection.after(interaction);const head=interaction.querySelector('.life-section-head'),label=head?.querySelector('.life-label'),card=interaction.querySelector('.life-together-card'),done=mine.filter(x=>x.done).length+theirs.filter(x=>x.done).length;if(label)label.remove();if(card){card.insertAdjacentHTML('afterbegin',`<button type="button" class="life-challenge-entry" data-life-open="challenge"><span>${icon('heart')}</span><div><small>今日默契</small><b>${esc(challengeStateCopy(challenge))}</b></div>${icon('caret-right')}</button>${mineReady&&!interactionComplete?'<div class="life-interaction-waiting" role="status"><i class="ph ph-heart"></i><span>我这边已经完成啦<br><small>等 TA 的回应到达后一起庆祝</small></span></div>':''}<div class="life-interaction-subhead"><span>今天的小互动</span><b>${done} / 8</b></div>`);}}
    const compactInteraction=interactionSlot?.querySelector('.life-today-together'),compactInteractionHead=compactInteraction?.querySelector('.life-section-head');if(compactInteractionHead){compactInteractionHead.insertAdjacentHTML('beforeend',`<span class="life-interaction-streak-inline">${icon('heart')}<span>连续 <b>${streak}</b> 天</span></span>`);compactInteraction?.querySelector('.life-interaction-streak')?.remove();}
...
openSheet(`<div class="life-travel-page life-travel-timeline-page"><header><button type="button" class="life-travel-back" ${view==='add'?'data-travel-back-timeline aria-label="返回旅行记录"':'data-sheet-close aria-label="返回小事"'}>${icon('caret-left')}</button><div><b>${view==='add'?'新的旅行':'旅行记录'}</b><small>${view==='add'?'只要几步，收好这一站':'去过和想去，都留在这里'}</small></div>${view==='add'?'<span class="life-travel-header-spacer"></span>':`<button type="button" class="life-travel-header-add" data-travel-open-add aria-label="新增旅行">${icon('plus')}</button>`}</header><main>${content}</main></div>`, 'life-sheet-travel');
  ...
  function toggleLifeTodo(id) {
    ...
    if(ok&&!wasDone)requestCompanionReaction('todo',`todo:${id}`);
    return !!ok;
  }
  ...
  document.addEventListener('visibilitychange', () => { if(document.hidden)activeSheetDraft?.flush();else { if(syncTimeAtmosphere())render(); showDeferredReactionNotice(); maybeOfferReview(); } });
  ...
  window.addEventListener('puffer-state-change', () => {render();syncTodayInteractionCelebration();refreshOpenMessageSheet();maybeOfferReview();});
  window.addEventListener('puffer-new-messages', event => showLiveMessageNotice(event.detail));
  window.addEventListener('puffer-remote-changes', event => {registerRemoteUpdates(event.detail?.changes);processRemoteCompanionReactions(event.detail?.changes);syncTodayInteractionCelebration();});
