(() => {
  ...
  const navButton = page => `<button class="${page==='today'?'active':''}" data-life-tab="${page}">${page==='today'?`<img class="life-nav-pet" src="assets/${pagePet(page)}" alt="">`:icon(navMeta[page].icon)}<span>${navMeta[page].label}</span></button>`;
  ...
  function positionCompanionNudge() { const nudge=document.querySelector('#lifeCompanionNudge'),pet=document.querySelector('#lifeCompanionFloat');if(!nudge||!pet)return;const r=pet.getBoundingClientRect(),w=nudge.offsetWidth||218,h=nudge.offsetHeight||56,gap=8,leftSide=r.left<window.innerWidth/2,rawLeft=leftSide?r.right+gap:r.left-w-gap,left=Math.max(8,Math.min(window.innerWidth-w-8,rawLeft)),top=Math.max(72,Math.min(window.innerHeight-h-104,r.top+(r.height-h)/2));nudge.style.left=`${Math.round(left)}px`;nudge.style.top=`${Math.round(top)}px`;nudge.style.right='auto';nudge.style.bottom='auto'; }
  function emitCompanionExpression(node) { const tone=node.dataset.tone||'quiet', glyphs={quiet:['·','☺'],missing:['♥','♡'],happy:['✦','♥'],celebrate:['✦','♥'],goodnight:['z','⋆'],days:['✦','•'],things:['☺','✦'],home:['♥','⌂']}[tone]||['·'];const accent=document.createElement('span');accent.className='life-companion-accent';accent.textContent=glyphs[Math.floor(Math.random()*glyphs.length)];accent.style.setProperty('--life-accent-shift',`${Math.round((Math.random()-.5)*30)}px`);node.append(accent);setTimeout(()=>accent.remove(),1450); }
  function ensureCompanionBlink(node) { if(!node)return; const revision='20260811-pet-blink-light-1',frames={'puffer-state-quiet.webp':'puffer-state-quiet-blink-v2.webp','puffer-state-missing.webp':'puffer-state-missing-blink-v2.webp','puffer-state-happy.webp':'puffer-state-happy-blink-v2.webp','puffer-state-celebrate.webp':'puffer-state-celebrate-blink-v2.webp','puffer-state-goodnight.webp':'puffer-state-goodnight-blink-v2.webp'},prepare=asset=>{const file=frames[asset];if(!file||node._blinkReady?.[file]||node._blinkLoading===file)return;node._blinkLoading=file;node._blinkReady=node._blinkReady||{};const preload=new Image();preload.onload=()=>{node._blinkReady[file]=true;node._blinkLoading='';};preload.onerror=()=>{node._blinkLoading='';};preload.src=`assets/${file}?v=${revision}`;};prepare(node._pet?.asset);if(node._blinkTimer)return;const blink=()=>{const image=node.querySelector('img'),asset=node._pet?.asset,closed=frames[asset];prepare(asset);if(!closed||!node._blinkReady?.[closed]||node.classList.contains('is-dragging')||node.classList.contains('is-swimming')||document.hidden||!image)return;image.src=`assets/${closed}?v=${revision}`;setTimeout(()=>{if(node._pet?.asset===asset&&!node.classList.contains('is-swimming'))image.src=`assets/${asset}`;},135);};node._blinkTimer=setInterval(blink,11800);setTimeout(blink,7600); }
  function ensureCompanionIdle(node) { if(!node||node._idleTimer)return; const animate=()=>{if(node.classList.contains('is-dragging')||document.hidden||!node._pet)return;const image=node.querySelector('img'),tone=node.dataset.tone||'quiet';node.classList.remove('is-idle-action');void node.offsetWidth;node.classList.add('is-idle-action');if(Math.random()<.25)setTimeout(()=>emitCompanionExpression(node),180);if(tone==='goodnight'){setTimeout(()=>node.classList.remove('is-idle-action'),850);return;}const frames=['puffer-swim-1.webp','puffer-swim-2.webp','puffer-swim-3.webp'];let frame=0;clearInterval(node._idleFrameTimer);image.src=`assets/${frames[frame]}`;node._idleFrameTimer=setInterval(()=>{frame=(frame+1)%frames.length;image.src=`assets/${frames[frame]}`;},135);setTimeout(()=>{clearInterval(node._idleFrameTimer);image.src=`assets/${node._pet.asset}`;image.dataset.asset=node._pet.asset;node.classList.remove('is-idle-action');},620);};node._idleTimer=setInterval(animate,21000);setTimeout(animate,12000); }
  function renderCompanionNudge(s) { const old=document.querySelector('#lifeCompanionNudge'); old?.remove(); if(activeLifeTab!=='today'||mask.classList.contains('show'))return; const greeting=scheduledGreeting(s), greetingKey=`puffer-companion-nudge:${dayKey()}:${greeting.id}`, cue=!localStorage.getItem(greetingKey)?greeting:companionCue(s), key=cue===greeting?greetingKey:`puffer-companion-nudge:${dayKey()}:${cue.id}`; if(localStorage.getItem(key))return; const node=document.createElement('button');node.type='button';node.id='lifeCompanionNudge';node.className='life-companion-nudge';if(cue===greeting)node.dataset.lifeGreetingSlot=greetingSlot();node.innerHTML=`<span>${esc(cue.kind==='memory'?'重新遇见':cue.title||'胖头鱼提醒')}</span><b>${esc(cue.kind==='memory'?cue.title:cue.text)}</b>`;node.addEventListener('click',()=>{localStorage.setItem(key,'1');node.remove();companionResponseSheet(cue);});document.body.append(node);requestAnimationFrame(()=>{positionCompanionNudge();node.classList.add('show');});setTimeout(()=>{node.classList.remove('show');setTimeout(()=>node.remove(),220);},9000); }
  function renderCompanionFloat(s) { if(!s) return; let pet=companionPet(s); let node=document.querySelector('#lifeCompanionFloat'); if(!node){node=document.createElement('div');node.id='lifeCompanionFloat';node.className='life-companion-float';node.innerHTML='<img alt="">';document.body.append(node);const read=()=>{try{return JSON.parse(localStorage.getItem('puffer-companion-float-position')||'');}catch{return null;}};const place=pos=>{const width=node.offsetWidth||86,height=node.offsetHeight||86,edge=8,y=Math.max(72,Math.min(window.innerHeight-height-104,Math.round((window.innerHeight-height)*Number(pos?.y??.58))));node.style.left=`${pos?.side==='left'?edge:Math.max(edge,window.innerWidth-width-edge)}px`;node.style.top=`${y}px`;};node._place=place;requestAnimationFrame(()=>place(read()||{side:'right',y:.58}));const stopSwimming=()=>{clearInterval(node._swimTimer);node._swimTimer=null;node.classList.remove('is-swimming');if(node._pet){const image=node.querySelector('img');image.src=`assets/${node._pet.asset}`;image.dataset.asset=node._pet.asset;image.alt=`${node._pet.label}胖头鱼`;}};const startSwimming=()=>{if(window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)return;const image=node.querySelector('img'),frames=['puffer-swim-1.webp','puffer-swim-2.webp','puffer-swim-3.webp'];let frame=0;clearInterval(node._swimTimer);node.classList.add('is-swimming');image.src=`assets/${frames[frame]}`;node._swimTimer=setInterval(()=>{frame=(frame+1)%frames.length;image.src=`assets/${frames[frame]}`;},120);};let drag=null;node.addEventListener('pointerdown',e=>{drag={x:e.clientX,y:e.clientY,left:node.offsetLeft,top:node.offsetTop};node.setPointerCapture?.(e.pointerId);node.classList.add('is-dragging');startSwimming();});node.addEventListener('pointermove',e=>{if(!drag)return;node.style.left=`${Math.max(8,Math.min(window.innerWidth-node.offsetWidth-8,drag.left+e.clientX-drag.x))}px`;node.style.top=`${Math.max(72,Math.min(window.innerHeight-node.offsetHeight-104,drag.top+e.clientY-drag.y))}px`;});const finishDrag=()=>{if(!drag)return;const pos={side:node.offsetLeft+node.offsetWidth/2<window.innerWidth/2?'left':'right',y:node.offsetTop/Math.max(1,window.innerHeight-node.offsetHeight)};localStorage.setItem('puffer-companion-float-position',JSON.stringify(pos));node.classList.remove('is-dragging');stopSwimming();place(pos);drag=null;};node.addEventListener('pointerup',finishDrag);node.addEventListener('pointercancel',finishDrag);window.addEventListener('resize',()=>place(read()||{side:'right',y:.58}));}const celebrationKey=`puffer-companion-celebration-${dayKey()}`;if(new Date().getHours()<20&&s.interactionHistory?.[dayKey()]&&!sessionStorage.getItem(celebrationKey)){sessionStorage.setItem(celebrationKey,'1');pet={asset:'puffer-state-celebrate.webp',label:'庆祝',tone:'celebrate'};clearTimeout(node._celebrationTimer);node._celebrationTimer=setTimeout(()=>renderCompanionFloat(state()),1800);}node._pet=pet;node.dataset.tone=pet.tone||'quiet';const image=node.querySelector('img');if(!node.classList.contains('is-swimming')&&image.dataset.asset!==pet.asset){image.dataset.asset=pet.asset;image.src=`assets/${pet.asset}`;image.alt=`${pet.label}胖头鱼`;node.classList.remove('is-updated');void node.offsetWidth;node.classList.add('is-updated');}}
